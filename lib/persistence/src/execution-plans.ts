...
function toPlanSummary(row: ExecutionPlanRow): ExecutionPlanSummary {
...
export class ExecutionPlanRepository {
  ...
  async listPlanSummaries(input: {
    ...
    const result = await this.client.query<ExecutionPlanRow>(
      ...
      WHERE execution_plans.workspace_id = $1
        AND ($2::text IS NULL OR execution_plans.status = $2)
      ORDER BY execution_plans.created_at DESC, execution_plans.plan_id DESC
      LIMIT $3`,
      [resolvedWorkspaceId, input.status ?? null, input.limit ?? 50]
    );

    return result.rows.map((row) => toPlanSummary(row));
  }
